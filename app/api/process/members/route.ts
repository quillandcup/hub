...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Note: on_hiatus is always 0 here — hiatus (member_hiatus_history) and
    // gift/180_program (member_status_overrides) overrides are applied
    // inside reprocess_members_atomic after this snapshot is taken, so this
    // breakdown reflects pre-override Kajabi-derived status only.
    return NextResponse.json({
      success: true,
