...
export default function BulkMergeMemberModal({ members, isOpen, onClose }: BulkMergeMemberModalProps) {
  ...
  useEffect(() => {
    if (!secondaryIds || !primaryId) { setExternalStatus(null); return; }
    setExternalLoading(true);
    fetch(`/api/admin/members/external-status?ids=${primaryId},${secondaryIds}`)
      .then(r => r.json())
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Merge Members" maxWidth="lg">
      <div className="space-y-5">
        ...
        {!externalLoading && externalStatus && primary && secondaries.some(s => hasExternalNotes(externalStatus[s.id])) && (() => {
          const notable = secondaries.filter(s => hasExternalNotes(externalStatus[s.id]));
          const actionNeeded = notable.some(s => needsActionBeforeMerge(externalStatus[primary.id], externalStatus[s.id]));
          return (
            <div
              className={
                actionNeeded
                  ? "rounded-lg bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-700 p-3 text-xs space-y-3"
                  : "rounded-lg bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 p-3 text-xs space-y-3"
              }
            >
              <p className={actionNeeded ? "font-semibold text-red-800 dark:text-red-300" : "font-semibold text-slate-700 dark:text-slate-300"}>
              ...
              {notable.map(s => (
                <MergeExternalNotice
                  ...
                  secondary={s}
                  primaryStatus={externalStatus[primary.id]}
                  secondaryStatus={externalStatus[s.id]}
                />
              ))}
            </div>
          );
