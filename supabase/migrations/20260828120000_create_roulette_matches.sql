-- Wheel of Wonder (formerly "Hedgie Roulette") match tracking: records the
-- 3-person Slack room (spinner + matched member + bot) created when someone
-- spins, and whether either human actually replied in it (a "confirmed
-- connection"), detected via the Slack Events webhook. Local layer
-- (operational data owned by this app, plain CRUD -- not reprocessed from
-- Bronze/Silver).
...
-- app/(member)/wheel-of-wonder/actions.ts, scoped to the acting member's own
