-- Fix member reprocessing to handle email changes in Kajabi
--
-- Bug: reprocess_members_atomic UPSERTs on email. When a member's email changes
-- in Kajabi, the incoming record has (kajabi_id=X, email=new@x.com). Since no row
-- exists with email=new@x.com, it inserts a NEW row instead of updating the
-- existing (kajabi_id=X, email=old@x.com) record. The old row persists unmodified.
--
-- Fix: Match by kajabi_id first (UPDATE including email), then fall back to
-- email UPSERT for new contacts and staff-only members. Also clean up stale
-- duplicates left by the old broken behavior.
--
-- IMPORTANT: prickle_attendance.member_id has ON DELETE CASCADE, so deleting
-- members would cascade-delete their attendance. Before deleting a stale duplicate,
-- we reassign its prickle_attendance records to the original (canonical) member.
