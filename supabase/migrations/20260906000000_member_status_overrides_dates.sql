-- member_status_overrides.starts_at/expires_at were TIMESTAMPTZ, but the
-- only UI that writes them (MemberOverrideForm) is a plain <input
-- type="date"> with no time-of-day picker -- every value ever written is a
-- bare "YYYY-MM-DD" string. PostgREST/Postgres interprets that as UTC
-- midnight, so a staffer entering "April 6" produced a real instant of
-- 2026-04-06T00:00:00Z, which then renders as "April 5" once the timeline
-- (lib/membership-timeline.ts) formats it with .toLocaleDateString() in any
-- timezone behind UTC. Switching to DATE matches the actual granularity of
-- the data and the pattern already used for member_hiatus_history.start_date
-- and program_cohorts.starts_at, which don't have this bug.
--
-- The USING clause reads the existing UTC-midnight instants back out in UTC
-- (not the session timezone) so every row keeps the calendar date staff
-- actually typed.
ALTER TABLE member_status_overrides
  ALTER COLUMN starts_at TYPE DATE USING (starts_at AT TIME ZONE 'UTC')::date,
  ALTER COLUMN starts_at SET DEFAULT CURRENT_DATE,
  ALTER COLUMN expires_at TYPE DATE USING (expires_at AT TIME ZONE 'UTC')::date;

-- Step 4 (gift/direct_stripe override window check) now compares DATEs the
-- same way Step 4b (hiatus) and Step 4a (program cohorts) already do --
-- otherwise identical to 20260904140000_add_direct_stripe_override_type.sql.
