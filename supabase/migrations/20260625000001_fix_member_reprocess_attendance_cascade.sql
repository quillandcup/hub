-- Fix: reprocess_members_atomic must reassign prickle_attendance before deleting
-- stale duplicate members, or the ON DELETE CASCADE wipes attendance history.
--
-- Root cause: 20260621000001 added a DELETE FROM members step to clean up
-- email-change duplicates. But prickle_attendance.member_id has ON DELETE CASCADE,
-- so deleting the stale member also deleted all its attendance records.
--
-- Fix: Before deleting each stale duplicate, UPDATE prickle_attendance to point
-- its records at the canonical (original) member instead.
