-- Add kajabi_tags to members (Silver), sourced from the Kajabi contact_tags
-- relationship now resolved during Bronze import (lib/kajabi/client.ts
-- fetchContactsPaginated requests include=tags and attaches contact.tags).
-- Powers the Outreach page (/admin/outreach), which lists leads tagged
-- "Ideal Hedgie" in Kajabi.
ALTER TABLE members
  ADD COLUMN kajabi_tags TEXT[] NOT NULL DEFAULT '{}';

-- GIN index so `kajabi_tags @> ARRAY['Ideal Hedgie']`-style lookups (used by
-- the Outreach page) don't scan every member.
CREATE INDEX idx_members_kajabi_tags ON members USING GIN (kajabi_tags);

COMMENT ON COLUMN members.kajabi_tags IS 'Kajabi contact tags (e.g. "Ideal Hedgie"), refreshed on every /api/process/members run. Not editable in-app.';

-- reprocess_members_atomic: thread kajabi_tags through the Step 2 UPDATE and
-- Step 3 INSERT/UPSERT, identical elsewhere to
-- 20260906000000_member_status_overrides_dates.sql.
