-- Add a 'direct_stripe' status override type, distinct from 'gift'.
--
-- 'gift' means a complimentary/free membership (hosting comp, etc.) —
-- semantically wrong for a member who IS paying, just through a channel
-- this app's sync never sees: an ad-hoc subscription set up directly in
-- Kajabi's Payments/Billing area (its newer commerce product, separate from
-- the classic Offers catalog kajabi_purchases/kajabi_offers sync from).
...
-- 20260904130000_program_cohorts_supersede_180_program.sql.
