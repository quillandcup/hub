-- Retire staff.user_id in favor of members.user_id as the sole, durable
-- login link. Every staff person already ends up with a members row
-- (process/members/route.ts synthesizes one even without Kajabi history),
-- so a second, separate link on staff was redundant and could drift from
-- the real one. Add staff.member_id as a maintained FK cache of the
-- staff<->member relationship (kept in sync by app/api/process/members
-- after each reprocess) instead of re-deriving it by email every time.
ALTER TABLE staff ADD COLUMN member_id UUID REFERENCES members(id) ON DELETE SET NULL;

-- One-time backfill by current email match (staff and members are both
-- internal tables we control, so this is a safe, one-shot reconciliation --
-- going forward member_id is maintained directly, not re-derived).
UPDATE staff s
SET member_id = m.id
FROM members m
WHERE lower(m.email) = lower(s.email)
  AND s.member_id IS NULL;

-- staff.user_id is redundant now that members.user_id is the durable link
-- (see reprocess_members_atomic fix below, which stops staff.user_id from
-- ever being needed to populate it).
DROP INDEX IF EXISTS idx_staff_user_id;
ALTER TABLE staff DROP COLUMN user_id;

-- Fix reprocess_members_atomic: previously it unconditionally overwrote
-- members.user_id from the incoming payload on every run (payload only ever
-- carried a value via the now-retired staff.user_id), silently wiping any
-- link established directly on a member (e.g. via the admin Member picker
-- or an invite). Preserve the existing value when the payload doesn't
-- supply one. Identical to 20260908170000_add_kajabi_tags_to_members.sql
-- except for the three `user_id` write sites below.
...
BEGIN
  ...
  SET
    ...
    user_id = COALESCE((nd.value->>'user_id')::uuid, m.user_id),
    ...
    kajabi_tags = ARRAY(SELECT jsonb_array_elements_text(COALESCE(nd.value->'kajabi_tags', '[]'::jsonb))),
    updated_at = NOW()
  ...
  INSERT INTO members (
    ...
    photo_url, bio, instagram_url, facebook_url, twitter_url, kajabi_tags
  )
  SELECT
    value->>'email',
    value->>'name',
    ...
    value->>'facebook_url',
    value->>'twitter_url',
    ARRAY(SELECT jsonb_array_elements_text(COALESCE(value->'kajabi_tags', '[]'::jsonb)))
  ...
  ON CONFLICT (email) DO UPDATE SET
    ...
    kajabi_tags = EXCLUDED.kajabi_tags,
    updated_at = NOW();

  -- Step 4: Apply active gift/direct_stripe overrides — this is what makes
  ...
  -- for status purposes today). starts_at/expires_at are DATE (see this
  -- migration), so compared against current_date like hiatus/program cohorts.
  WITH active_override AS (
    SELECT DISTINCT ON (member_id) member_id, override_type
    FROM member_status_overrides
    WHERE override_type IN ('gift', 'direct_stripe')
      AND current_date BETWEEN starts_at AND COALESCE(expires_at, 'infinity'::date)
  ...
  -- way a 'gift'/'direct_stripe' override does. Placed before 4b (hiatus) so
  -- hiatus still wins via ordering, same as Step 4.
  UPDATE members m
  SET status = 'active', updated_at = NOW()
  ...
  -- purchase never produces — without this, a member Step 4a/4c just
