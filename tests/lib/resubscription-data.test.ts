 ...
 * going through the HTTP API layer. Same bronze table seed data as the
 * API route tests, but verifies the function works standalone with a
 * Supabase client — which is what the Server Component uses.
 */
...
describe("fetchResubscriptionsData()", () => {
  ...
  beforeAll(async () => {
    ...
    await supabase
      ...
      .insert([customerA, customerB, customerC]);
