...
describe('Slack Import Idempotency', () => {
  ...
  beforeAll(async () => {
    ...
    await supabase.schema('bronze').from('slack_reactions').delete().like('channel_id', 'TEST_CHANNEL_%')
    await supabase.schema('bronze').from('slack_messages').delete().like('channel_id', 'TEST_CHANNEL_%')
    await supabase.schema('bronze').from('slack_channels').delete().like('channel_id', 'TEST_CHANNEL_%')
    await supabase.schema('bronze').from('slack_users').delete().like('user_id', 'TEST_USER_%')
  })
